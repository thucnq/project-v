...
func TestNewStandardLibraryJsonUtils(t *testing.T) {
	tests := []struct {
		name string
		...
		{
			name: "OK",
			want: NewStandardLibraryJsonUtils(),
		},
	}
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				if got := NewStandardLibraryJsonUtils(); !reflect.DeepEqual(
					got, tt.want,
				) {
