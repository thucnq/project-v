...
import (
	...
	errors2 "project-v/pkg/errors"
	"project-v/pkg/l/config"
...
func Error(err error) zapcore.Field {
	...
	if xerr, ok := err.(*errors2.APIError); ok {
		errStringify, _ := xerr.MarshalJSON()
		return String("error", string(errStringify))
	}
...
func newWithName(
	name string, sentryCfg *sentry.Configuration, opts ...zap.Option,
) Logger {
	if name == "" {
...
func init() {
	...
	if errPattern != "" {
		ll.Fatal(
			"Unable to parse LOG_LEVEL. Please set it to a proper value.",
			String("invalid", errPattern),
		)
	}
	...
	case "PUT":
		...
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			...
			err = enc.Encode(
				errorResponse{
					Error: fmt.Sprintf(
						"Request body must be valid JSON: %v", err,
					),
				},
			)
			if err != nil {
